...
截至 v3.20.3，Codex 供应商应按**具体端点的协议**配置：

- **原生 Responses**：DeepSeek、智谱 GLM、千问、MiniMax、小米 MiMo、LongCat、Kimi 开放平台与 Kimi For Coding 等官方预设可直连，无需为了协议转换开启本地路由；也可经标准代理转发。
- **Chat Completions**：百度千帆 Coding Plan、腾讯 Token Plan、QwenCloud For Coding、StepFun、百灵、ModelScope 等 Chat 端点仍需协议转换。使用时须启动[本地路由](../4-proxy/4.1-service.md)并启用 Codex 接管；聚合平台以所选预设和端点为准。

> **旧卡不会自动升级**：供应商保存的是创建时的预设快照。要让旧版 Kimi、DeepSeek、GLM 等卡片使用直连，建议重新添加最新版预设；手动迁移时，需同时确认上游格式为 Responses、Base URL 和模型目录匹配厂商的 Responses 端点。详见 [v3.20.3 更新说明](/zh/changelog/3.20.3)。

近期还新增了腾讯云 Token Plan、QwenCloud、AICodeWith、9527CODE 和 SoleAPI 预设，各应用的可用列表以添加窗口为准。
