...
As of v3.20.3, configure Codex providers according to the **protocol of the specific endpoint**:

- **Native Responses**: official presets for DeepSeek, Zhipu GLM, Qwen, MiniMax, Xiaomi MiMo, LongCat, the Kimi Open Platform and Kimi For Coding can connect directly, without local routing for protocol conversion. Standard proxy routing remains optional.
- **Chat Completions**: Chat endpoints such as Baidu Qianfan Coding Plan, Tencent Token Plan, QwenCloud For Coding, StepFun, BaiLing and ModelScope still need conversion. Start [local routing](../4-proxy/4.1-service.md) and enable Codex takeover; for aggregators, follow the selected preset and endpoint.

> **Existing cards do not upgrade automatically**: providers store a snapshot of their original preset. For direct connections with older Kimi, DeepSeek or GLM cards, adding the latest preset is recommended. When migrating manually, check that the format is Responses and that the Base URL and model catalog match the vendor's Responses endpoint. See the [v3.20.3 release notes](/en/changelog/3.20.3).

Recent additions include Tencent Cloud Token Plan, QwenCloud, AICodeWith, 9527CODE and SoleAPI. Available presets vary by app; refer to the add-provider dialog.
