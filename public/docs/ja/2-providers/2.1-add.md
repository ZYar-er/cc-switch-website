...
v3.20.3 時点では、**利用するエンドポイントのプロトコル**に合わせて Codex プロバイダーを設定してください。

- **ネイティブ Responses**：DeepSeek、Zhipu GLM、Qwen、MiniMax、Xiaomi MiMo、LongCat、Kimi オープンプラットフォーム、Kimi For Coding などの公式プリセットは直接接続でき、プロトコル変換のためのローカルルーティングは不要です。通常のプロキシ経由での転送も選択できます。
- **Chat Completions**：Baidu Qianfan Coding Plan、Tencent Token Plan、QwenCloud For Coding、StepFun、BaiLing、ModelScope などの Chat エンドポイントには変換が必要です。[ローカルルーティング](../4-proxy/4.1-service.md)を起動し、Codex の引き継ぎを有効にしてください。集約サービスは選択したプリセットとエンドポイントに従ってください。

> **既存カードは自動更新されません**：作成時のプリセットのスナップショットが保存されます。旧版の Kimi、DeepSeek、GLM カードを直接接続に移行するには、最新のプリセットを追加し直すことを推奨します。手動の場合は、上流形式が Responses で、Base URL とモデルカタログもベンダーの Responses エンドポイントに対応していることを確認してください。詳細は [v3.20.3 更新情報](/ja/changelog/3.20.3)を参照してください。

最近は Tencent Cloud Token Plan、QwenCloud、AICodeWith、9527CODE、SoleAPI も追加されました。各アプリのプリセットは追加ダイアログで確認してください。
